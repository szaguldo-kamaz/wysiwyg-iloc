...
#  key i : toggle bright/dark color mode
...
#brightmode = False  # standard - dark background, bright object
brightmode = True  # bright background, dark objects
...
windowbackgroundcolor_dark = '#000000'
windowbackgroundcolor_bright = '#FFFFFF'
linecolor1_dark = '#FFFFFF'
linecolor2_dark = '#00FF00'
linecolor3_dark = '#FF00FF'
linecolor1_bright = '#000000'
linecolor2_bright = '#008000'
linecolor3_bright = '#800080'
offset_text_color_dark = '#FFFFFF'
offset_text_color_bright = '#000000'
...
if brightmode:
    windowbackgroundcolor = windowbackgroundcolor_bright
    linecolor1 = linecolor1_bright
    linecolor2 = linecolor2_bright
    linecolor3 = linecolor3_bright
    offset_text_color = offset_text_color_bright
else:
    windowbackgroundcolor = windowbackgroundcolor_dark
    linecolor1 = linecolor1_dark
    linecolor2 = linecolor2_dark
    linecolor3 = linecolor3_dark
    offset_text_color = offset_text_color_dark
...
if wilobj.calibrate_from_file(calibdata_filename):
    ...
    print(f"Cannot load calibration data from: {calibdata_filename}, using default values.")
    xoffset_world = 0.0
    yoffset_world = 0.0
    zoffset_world = 0.0
    yawoffset_world   = 0.0
...
gobj_playarea_border = graph.draw_rectangle( [1, 1], [wilobj.config.playareapixels[0] - 2, wilobj.config.playareapixels[1] - 1], line_color=linecolor1, line_width=3)

gobj_playarea_lines_color1 = []
gobj_playarea_lines_color2 = []
gobj_playarea_lines_color3 = []
# color2
for no in range(0,4):
    gobj_playarea_lines_color2.append(graph.draw_line( ((wilobj.config.playareapixels[0]/2) - (2*no*100) - 100, 0) , ((wilobj.config.playareapixels[0]/2) - (2*no*100)-100,wilobj.config.playareapixels[1]), color=linecolor2))
    gobj_playarea_lines_color2.append(graph.draw_line( ((wilobj.config.playareapixels[0]/2) + (2*no*100) + 100, 0) , ((wilobj.config.playareapixels[0]/2) + (2*no*100)+100,wilobj.config.playareapixels[1]), color=linecolor2))
for no in range(0,3):
    gobj_playarea_lines_color2.append(graph.draw_line( (0, (wilobj.config.playareapixels[1]/2) - (2*no*100) - 100, 0) , (wilobj.config.playareapixels[0], (wilobj.config.playareapixels[1]/2) - (2*no*100) - 100), color=linecolor2))
    gobj_playarea_lines_color2.append(graph.draw_line( (0, (wilobj.config.playareapixels[1]/2) + (2*no*100) + 100, 0) , (wilobj.config.playareapixels[0], (wilobj.config.playareapixels[1]/2) + (2*no*100) + 100), color=linecolor2))
# color3 should be above color2 lines
for no in range(1,4):
    gobj_playarea_lines_color3.append(graph.draw_line( ((wilobj.config.playareapixels[0]/2) - (2*no*100),0) , ((wilobj.config.playareapixels[0]/2) - (2*no*100),wilobj.config.playareapixels[1]), color=linecolor3))
    gobj_playarea_lines_color3.append(graph.draw_line( ((wilobj.config.playareapixels[0]/2) + (2*no*100),0) , ((wilobj.config.playareapixels[0]/2) + (2*no*100),wilobj.config.playareapixels[1]), color=linecolor3))
for no in range(1,3):
    gobj_playarea_lines_color3.append(graph.draw_line( (0, (wilobj.config.playareapixels[1]/2) - (2*no*100)) , (wilobj.config.playareapixels[0], (wilobj.config.playareapixels[1]/2) - (2*no*100)), color=linecolor3))
    gobj_playarea_lines_color3.append(graph.draw_line( (0, (wilobj.config.playareapixels[1]/2) + (2*no*100)) , (wilobj.config.playareapixels[0], (wilobj.config.playareapixels[1]/2) + (2*no*100)), color=linecolor3))
# middle cross is a bit thicker, and color1
gobj_playarea_lines_color1.append(graph.draw_line( (wilobj.config.playareapixels[0]/2, 0), (wilobj.config.playareapixels[0]/2, wilobj.config.playareapixels[1]), color=linecolor1, width=2))
gobj_playarea_lines_color1.append(graph.draw_line( (0, wilobj.config.playareapixels[1]/2), (wilobj.config.playareapixels[0], wilobj.config.playareapixels[1]/2), color=linecolor1, width=2))
...
while True:
    ...
    if capture_bgflash_counter > 0:
        ...
        if capture_bgflash_counter == 0:
            if brightmode:
                graph.update(background_color = windowbackgroundcolor_bright)
            else:
                graph.update(background_color = windowbackgroundcolor_dark)
            needoffsetupdate = True
        ...
        for trackername in trackernames:
            ...
            if trackedobjs[trackername]['pospixel'] != None:
                ...
                if individualtrackermode and individualtrackermode_whichtracker == trackername:
                    trackedobjs[trackername]['plotselectedobj'] = graph.draw_circle(trackedobjs[trackername]['pospixel'], trackedobjs[trackername]['plotsize'] + 40, line_color=linecolor1, line_width=5)
    ...
    # invert dark/bright color mode
    if event in [ "i:31", "i" ]:
        brightmode = not brightmode
        if brightmode:
            windowbackgroundcolor = windowbackgroundcolor_bright
            linecolor1 = linecolor1_bright
            linecolor2 = linecolor2_bright
        ...
        graph.Widget.itemconfig(gobj_playarea_border, outline=linecolor1)

        for lineobj in gobj_playarea_lines_color2:
            graph.Widget.itemconfig(lineobj, fill=linecolor2)
        for lineobj in gobj_playarea_lines_color3:
            graph.Widget.itemconfig(lineobj, fill=linecolor3)
        for lineobj in gobj_playarea_lines_color1:
            graph.Widget.itemconfig(lineobj, fill=linecolor1)
    ...
    if event in [ "BackSpace:22", "BackSpace:8" ]:
        needoffsetupdate = True
        individualtrackermode = False
...
#            individualtrackermode = False
    ...
    if needoffsetupdate:
        ...
        offset_label = graph.draw_text(offset_text, [20, 20], color=offset_text_color, font=statusfont, text_location = sg.TEXT_LOCATION_LEFT)
